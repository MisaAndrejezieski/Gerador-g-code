...
# ==============================================
# SISTEMA DE APRENDIZADO MÁQUINA
# ==============================================
...
class LearningSystem:
    def __init__(self):
        ...
        self.dataset_path = "learning_dataset.csv"
        self.model = None
        self.load_model()
    ...
    def extract_features(self, image_array, params):
        ...
        try:
            ...
            features['contrast'] = np.max(image_array) - np.min(image_array)
            ...
            # Histogram features
            hist, _ = np.histogram(image_array, bins=10)
            features.update({f'hist_bin_{i}': val for i, val in enumerate(hist)})
            
            # Parâmetros do usuário
            features['wood_type'] = {'soft': 0, 'medium': 1, 'hard': 2}[params.get('wood_type', 'medium')]
            features['grain_direction'] = {'horizontal': 0, 'vertical': 1, 'diagonal': 2}[params.get('grain_direction', 'horizontal')]
            features['profundidade_max'] = params.get('profundidade_max', 4.0)
            features['passo'] = params.get('passo', 1.0)
    ...
    def add_feedback(self, image_array, params, z_map_result, user_rating, user_notes=""):
        ...
        try:
            ...
            feedback_entry = {
                ...
                'z_map_stats': {
                    'mean_depth': np.mean(z_map_result),
                    'max_depth': np.max(z_map_result),
                    'std_depth': np.std(z_map_result)
                }
            }
    ...
    def train_model(self):
        """Treina modelo com dados acumulados"""
        try:
            ...
            for entry in self.feedback_data:
                ...
                if features:
                    ...
                    for key in sorted(features.keys()):
                        if key not in ['wood_type', 'grain_direction']:
                            feature_vector.append(float(features[key]))
                    
                    X.append(feature_vector)
                    y.append(entry['user_rating'])
    ...
    def predict_optimal_params(self, image_array, current_params):
        ...
        try:
            ...
            if predicted_rating < 3:
                # Se predição é baixa, ajustar parâmetros
...
class EnhancedWoodCarvingApp:
    ...
    def create_main_tab(self, parent):
        ...
        canvas = tk.Canvas(parent, bg='#DEB887', highlightthickness=0)
        scrollbar = ttk.Scrollbar(parent, orient="vertical", command=canvas.yview)
        scrollable_frame = ttk.Frame(canvas, style='Wood.TFrame')
        ...
        ttk.Label(scrollable_frame, text="Configurações Principais", style='Wood.TLabel', font=('Segoe UI', 14, 'bold')).pack(anchor='w', pady=(10, 20))
        
        # Seleção de arquivo
        file_frame = ttk.Frame(scrollable_frame, style='Wood.TFrame')
        ...
        params_frame = ttk.Frame(scrollable_frame, style='Wood.TFrame')
        ...
        self.entry_largura = self.create_parameter(param_grid, "Largura da Peça (mm):", "200", 0)
        self.entry_altura = self.create_parameter(param_grid, "Altura da Peça (mm):", "150", 1)
        self.entry_profundidade = self.create_parameter(param_grid, "Profundidade Máxima (mm):", "4.0", 2)
        self.entry_passo = self.create_parameter(param_grid, "Passo da Ferramenta (mm):", "1.0", 3)
        self.entry_safe_z = self.create_parameter(param_grid, "Altura de Segurança (mm):", "5.0", 4)
        
        # Botões principais
        button_frame = ttk.Frame(scrollable_frame, style='Wood.TFrame')
        ...
        # Status
        self.status_label = ttk.Label(scrollable_frame, text="Pronto para criar entalhe em madeira", 
    ...
    def create_wood_tab(self, parent):
        ...
        ttk.Radiobutton(wood_frame, text="🔸 Dura (Carvalho, Mogno) - Corte lento", 
                       variable=self.tipo_madeira, value="hard", style='Wood.TLabel').pack(anchor='w', pady=2)
        
        # Direção do Veio
        ttk.Label(parent, text="Direção do Veio da Madeira:", style='Wood.TLabel').pack(anchor='w', pady=(15,5))
    ...
    def create_ai_tab(self, parent):
        ...
        modes = [
            ("🪵 Padrão Madeira (Recomendado)", "wood_default"),
            ("🎭 Imagens Sacras (Santos, Virgens)", "sacred"),
            ("👤 Retratos e Rostos", "portrait"), 
            ("🏞️ Paisagens e Natureza", "landscape"),
            ("📝 Texto e Documentos", "text"),
            ("🎨 Arte e Pinturas", "art")
        ]
    ...
    def atualizar_descricao_modo(self, *args):
        ...
        descricoes = {
            ...
            "sacred": "💡 Ideal para imagens religiosas - realça rostos e detalhes importantes",
            "portrait": "💡 Otimizado para retratos - foco em características faciais",
            "landscape": "💡 Para paisagens - destaca texturas naturais e relevos", 
            "text": "💡 Maximiza legibilidade de textos e documentos",
            "art": "💡 Preserva estilo artístico e pinceladas"
        }
    ...
    def previsualizar(self):
        ...
        try:
            ...
            plt.subplot(1, 2, 2)
            z_preview = (1 - img_array) * 4.0  # Preview simples
            ...
            self.status_label.config(text="Preview gerado - Verifique a janela de plots")
        ...
        try:
            ...
            if success:
                ...
                messagebox.showinfo("Sucesso!", 
                                  ...
                                  "Não esqueça de avaliar o resultado na aba 'Aprendizado'!")
            else:
                self.status_label.config(text="❌ Erro no processamento")
                
        except ValueError as e:
            self.status_label.config(text="❌ Erro nos parâmetros")
            messagebox.showerror("Erro", f"Verifique os valores dos parâmetros:\n{str(e)}")
        except Exception as e:
            self.status_label.config(text="❌ Erro no processamento")
    ...
    def processar_entalhe_madeira(self, img_path, params):
        ...
        try:
            ...
            # Gerar G-code
            gcode_filename = f"entalhe_madeira_{modo_ia}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.nc"
    ...
    def salvar_visualizacao(self, img_original, z_map, output_dir, params):
        ...
        try:
            ...
            fig.suptitle(f'Análise do Entalhe - Madeira {params["wood_type"].upper()} - Modo {params["modo_ia"]}', fontsize=16)
    ...
    def salvar_avaliacao(self):
        ...
        try:
            ...
            messagebox.showinfo("Sucesso", f"Avaliação {rating}⭐ salva com sucesso!\nO sistema irá aprender com seu feedback.")
...
💡 O sistema aprende com suas avaliações para sugerir
   parâmetros cada vez melhores!"""
        
        self.stats_label.config(text=text)
        
    def limpar_dados_aprendizado(self):
        """Limpa dados de aprendizado"""
        if messagebox.askyesno("Confirmar", "Tem certeza que deseja limpar todos os dados de aprendizado?"):
            self.learning_system.feedback_data = []
            self.learning_system.model = None
            try:
                if os.path.exists(self.learning_system.model_path):
                    os.remove(self.learning_system.model_path)
                if os.path.exists(self.learning_system.dataset_path):
                    os.remove(self.learning_system.dataset_path)
            except:
                pass
            self.atualizar_estatisticas()
            messagebox.showinfo("Sucesso", "Dados de aprendizado limpos!")
